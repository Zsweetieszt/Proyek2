...
void drawPlatform(int x, int y) {
    ...
    int rowCount = 4;
    int gap = tileSizeY / (rowCount * 10); // Celah proporsional
    ...
    for (int i = 0; i < rowCount; i++) {
        ...
        if (i % 2 == 0) {
            // Baris 1 & 3 = 3 bagian (kotak, persegi panjang, kotak)
            int totalGap = 2 * gap;
            int boxW = (tileSizeX - totalGap) / 4;
            int rectW = 2 * boxW; // agar tetap proporsional            
            ...
            // Baris 2 & 4 = 2 bagian (persegi panjang, persegi panjang)
